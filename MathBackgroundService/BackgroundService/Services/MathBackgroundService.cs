    ...
    private async Task EvaluateChoices()
    {
        ...
        using (IServiceScope scope = _serviceScopeFactory.CreateScope())
        {
            ...
            foreach (var userId in _data.Keys)
            {
                ...
                {
                    var player =  dbContext.Player.Where(p => p.UserId == userId).FirstOrDefault();
                ...
                {
                    await _mathQuestionHub.Clients.User(userId).SendAsync("EvaluateChoice", false, _currentQuestion.Answers[_currentQuestion.RightAnswerIndex]);
                }
